...
class Sale extends Model
{
    ...
    /**
     * Get cart data for a sale
     */
    public function get_cart(int $sale_id): ?array
    {
        $builder = $this->db->table('sales');
        $builder->select('cart_data');
        $builder->where('sale_id', $sale_id);
    ...
    /**
     * Get cart data by invoice number
     */
    public function get_cart_by_invoice(string $invoice_number): ?array
    {
        $builder = $this->db->table('sales');
        $builder->select('cart_data');
        $builder->where('invoice_number', $invoice_number);
    ...
    public function update($sale_id = null, $sale_data = null): bool
    {
        ...
        if ($success && !empty($sale_data['payments'])) {
            ...
            foreach ($sale_data['payments'] as $payment) {
                ...
                $cashflow_account_id = $payment['cashflow_account_id'] ?? null;
