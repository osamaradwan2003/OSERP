...
class Supplier extends Person
{
    ...
    {
        return $this->search($search, 0, 0, 'last_name', 'asc', true, $show_deleted);
    }

    /**
     * Perform a search on suppliers
     */
    public function search(string $search, ?int $rows = 25, ?int $limit_from = 0, ?string $sort = 'last_name', ?string $order = 'asc', ?bool $count_only = false, bool $show_deleted = false)
    {
        ...
        $builder->groupEnd();
        $builder->where('deleted', $show_deleted ? 1 : 0);
