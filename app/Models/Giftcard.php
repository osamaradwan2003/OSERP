...
class Giftcard extends Model
{
     ...
     * Gets gift cards
     */
    public function get_found_rows(string $search, bool $show_deleted = false): int
    {
        return $this->search($search, 0, 0, 'giftcard_number', 'asc', true, $show_deleted);
    }

    /**
     * Performs a search on giftcards
     */
    public function search(string $search, ?int $rows = 0, ?int $limit_from = 0, ?string $sort = 'giftcard_number', ?string $order = 'asc', ?bool $count_only = false, bool $show_deleted = false)
    {
        // Set default values
        if ($rows == null) $rows = 0;
        ...
        $builder->groupEnd();
        $builder->where('giftcards.deleted', $show_deleted ? 1 : 0);
