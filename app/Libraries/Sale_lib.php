...
class Sale_lib
{
    ...
    public function isCustomerCopy(): bool
    {
        return ($this->session->get('sales_print_customer_copy') == 'true'    // TODO: === ?
            || $this->session->get('sales_print_customer_copy') == '1');    // TODO: === ?
    }

    public function setCustomerCopy( bool $isCustomerCopy): void
    {
        $this->session->set('sales_print_customer_copy', $isCustomerCopy ? '1' : '0');
    }
     ...
     */
    public function add_payment(string $payment_id, string $payment_amount, int $cash_adjustment = CASH_ADJUSTMENT_FALSE, ?int $cashflow_account_id = null): void
    {
        $payments = $this->get_payments();
        $payment_key = $this->build_payment_key($payment_id, $cashflow_account_id);

        if (isset($payments[$payment_key])) {
            // payment_method already exists, add to payment_amount
            $payments[$payment_key]['payment_amount'] = bcadd($payments[$payment_key]['payment_amount'], $payment_amount);
        } else {
    ...
    private function build_payment_key(string $payment_id, ?int $cashflow_account_id = null): string
    {
        $account_id = $cashflow_account_id ?? 0;

        return $payment_id . '|' . $account_id;
    }
    ...
    public function delete_payment(string $payment_id): void
    {
        ...
        if ($cash_rounding) {
            $parts = explode('|', $decoded_payment_id);
            $decoded_payment_type = $parts[0];
            $decoded_account_id = $parts[1] ?? '0';

            if ($decoded_payment_type == lang('Sales.cash')) {    // TODO: === ?
                unset($payments[lang('Sales.cash_adjustment') . '|' . $decoded_account_id]);
            }

            if ($decoded_payment_type == lang('Sales.cash_adjustment')) {    // TODO: === ?
                unset($payments[lang('Sales.cash') . '|' . $decoded_account_id]);
            }
        }
        $this->set_payments($payments);
    }
     ...
     * @return bool
     */
    public function add_item(string &$item_id, int $item_location, string $quantity = '1', string &$discount = '0.0', int $discount_type = 0, int $price_mode = PRICE_MODE_STANDARD, ?int $kit_price_option = null, ?int $kit_print_option = null, ?string $price_override = null, ?string $description = null, ?string $serialnumber = null, ?int $sale_id = null, bool $include_deleted = false, ?bool $print_option = null, ?bool $line = null, ?bool $is_kit_item = false, ?string $kit_group_name= '', ?int $qt_in_kit=null): bool
    {
    ...
    public function add_item_kit(string $external_item_kit_id, int $item_location, float $discount, string $discount_type, bool $kit_price_option, bool $kit_print_option, ?string &$stock_warning): bool
    {
        ...
        foreach ($this->item_kit_items->get_info($item_kit_id) as $item_kit_item) {
            $result &= $this->add_item($item_kit_item['item_id'], $item_location, $item_kit_item['quantity'], $discount, $discount_type, PRICE_MODE_KIT, $kit_price_option, $kit_print_option, is_kit_item: true, kit_group_name: $item_kit_item['name'], qt_in_kit: $item_kit_item['quantity']);
    ...
    public function copy_entire_sale(int $sale_id): void
    {
        ...
        // Now load payments
        foreach ($this->sale->get_sale_payments($sale_id)->getResult() as $row) {
            $this->add_payment($row->payment_type, $row->payment_amount, $row->cash_adjustment, (int)($row->cashflow_account_id ?? 0));
        }
