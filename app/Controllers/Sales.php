...
class Sales extends Secure_Controller
{
    ...
    public function getSearch(): ResponseInterface
    {
        ...
        $sort = $this->sanitizeSortColumn(sales_headers(), $params['sort'], 'sale_id');
        ...
        $filters = $this->getCommonFilters([
            ...
            'is_valid_receipt' => $this->sale->is_valid_receipt($params['search'])
        ]);
        ...
        $sales = $this->sale->search($params['search'], $filters, $params['limit'], $params['offset'], $sort, $order);
        $total_rows = $this->sale->get_found_rows($params['search'], $filters);
        $payments = $this->sale->get_payments_summary($params['search'], $filters);
    ...
    /**
     * Sets PrintCustomerCopy flag. Used in app/Views/sales/register.php
     *
     * @return ResponseInterface|string
     * @noinspection PhpUnused
     */

    public function setCustomerInvoiceTemplate(): void
    {
        if($this->sale_lib->isCustomerCopy()) {
            $this->config['receipt_template'] = 'customer_invoice';
        }else {
            $this->config['receipt_template'] = 'receipt_default';
        }
    }

    public function postSetPrintCustomerCopy(): ResponseInterface
    {
        $this->sale_lib->setCustomerCopy($this->request->getPost('print_customer_copy') != 'false');
        $this->setCustomerInvoiceTemplate();
        return $this->response->setJSON(['success' => true, 'template'=>$this->config['receipt_template'], 'customer_copy' => $this->sale_lib->isCustomerCopy()]);
    }
    ...
    public function postAddPayment(): ResponseInterface|string
    {
        ...
        $cashflow_accounts = $this->cashflow_account->getActiveOptions();
        $cashflow_account_id = (int) ($this->request->getPost('cashflow_account_id', FILTER_SANITIZE_NUMBER_INT) ?? 0);

        $validationResult = $this->validateCashflowAccount($cashflow_accounts, $cashflow_account_id);
        if ($validationResult !== null) {
            return $this->_reload($validationResult);
        }

        $rules = $this->getPaymentValidationRules($payment_type);
        $messages = $this->getPaymentValidationMessages($payment_type);
        ...
        $paymentResult = $this->processPaymentByType($payment_type, $cashflow_account_id);
        if ($paymentResult !== null) {
            $data = array_merge($data, $paymentResult);
        }
    ...
    /**
     * Validate cashflow account selection.
     * @return array|null Error data array if validation fails, null on success.
     */
    private function validateCashflowAccount(array $cashflow_accounts, int $cashflow_account_id): ?array
    {
        if (empty($cashflow_accounts)) {
            return ['error' => lang('Sales.cashflow_no_active_accounts')];
        }
        if ($cashflow_account_id <= 0 || !array_key_exists($cashflow_account_id, $cashflow_accounts)) {
            return ['error' => lang('Sales.cashflow_account_required')];
        }
        return null;
    }
     ...
     */
    private function getPaymentValidationRules(string $payment_type): array
    {
        if ($payment_type === lang('Sales.giftcard')) {
            return ['amount_tendered' => 'trim|required'];
        }
        return ['amount_tendered' => 'trim|required|decimal_locale'];
    }

    /**
     * Get validation messages based on payment type.
     */
    private function getPaymentValidationMessages(string $payment_type): array
    {
        if ($payment_type === lang('Sales.giftcard')) {
            return ['amount_tendered' => lang('Sales.must_enter_numeric_giftcard')];
        }
        return ['amount_tendered' => lang('Sales.must_enter_numeric')];
    }
     ...
     */
    private function processPaymentByType(string $payment_type, int $cashflow_account_id): ?array
    {
        if ($payment_type === lang('Sales.giftcard')) {
            return $this->processGiftcardPayment($payment_type, $cashflow_account_id);
        }

        if ($payment_type === lang('Sales.rewards')) {
            return $this->processRewardsPayment($payment_type, $cashflow_account_id);
        }

        if ($payment_type === lang('Sales.cash')) {
            return $this->processCashPayment($payment_type, $cashflow_account_id);
        }

        return $this->processDefaultPayment($payment_type, $cashflow_account_id);
    }

    /**
     * Process giftcard payment.
     */
    private function processGiftcardPayment(string $payment_type, int $cashflow_account_id): array
    {
        $data = [];
        ...
        $giftcard_payment_key = $payment_type . '|' . $cashflow_account_id;
        $current_payments_with_giftcard = isset($payments[$giftcard_payment_key]) ? $payments[$giftcard_payment_key]['payment_amount'] : 0;
        ...
        if (isset($cur_giftcard_customer) && $cur_giftcard_customer != $customer_id && $cur_giftcard_customer != null) {
            ...
            $this->sale_lib->add_payment($payment_type, $amount_tendered, CASH_ADJUSTMENT_FALSE, $cashflow_account_id);
        }
    ...
    /**
     * Process rewards payment.
     */
    private function processRewardsPayment(string $payment_type, int $cashflow_account_id): array
    {
        $data = [];
        $customer_id = $this->sale_lib->get_customer();
        ...
        $payments = $this->sale_lib->get_payments();
        $rewards_payment_key = $payment_type . '|' . $cashflow_account_id;
        $current_payments_with_rewards = isset($payments[$rewards_payment_key]) ? $payments[$rewards_payment_key]['payment_amount'] : 0;
    ...
    /**
     * Process cash payment with adjustment handling.
     */
    private function processCashPayment(string $payment_type, int $cashflow_account_id): array
    {
        ...
        if ($cash_adjustment_amount <> 0) {
            ...
            $this->sale_lib->add_payment(lang('Sales.cash_adjustment'), $cash_adjustment_amount, CASH_ADJUSTMENT_TRUE, $cashflow_account_id);
        }
    ...
    /**
     * Process default payment (credit card, check, etc.).
     */
    private function processDefaultPayment(string $payment_type, int $cashflow_account_id): array
    {
        $amount_tendered = parse_decimals($this->request->getPost('amount_tendered'));
        $this->sale_lib->add_payment($payment_type, $amount_tendered, CASH_ADJUSTMENT_FALSE, $cashflow_account_id);
        return [];
    ...
    public function postComplete(): string    // TODO: this function is huge.  Probably should be refactored.
    {
        ...
        if ($data['amount_change'] > 0) {
            ...
            $cash_payment_key = $this->findPaymentKeyByType($data['payments'], lang('Sales.cash'));
            if ($cash_payment_key !== null) {
                $data['payments'][$cash_payment_key]['cash_refund'] = $data['amount_change'];
            } else {
                $default_cashflow_account_id = $this->getDefaultCashflowAccountId($this->cashflow_account->getActiveOptions());
                $payment = [
                    lang('Sales.cash') . '|' . ($default_cashflow_account_id ?? 0) => [
                        'payment_type'   => lang('Sales.cash'),
                        'payment_amount' => '0',
                        'cash_refund'    => $data['amount_change'],
                        'cash_adjustment' => CASH_ADJUSTMENT_FALSE,
                        'cashflow_account_id' => $default_cashflow_account_id
                    ]
                ];
        ...
        if ($this->sale_lib->is_invoice_mode()) {
            ...
            if ($sale_id == NEW_ENTRY && $this->sale->check_invoice_number_exists($invoice_number)) {
                ...
                $data['sale_id_num'] = $this->sale->save_value($sale_id, $data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $work_order_number, $quote_number, $sale_type, $data['payments'], $data['dinner_table'], $tax_details, $has_kit);
                ...
                if ($data['sale_id_num'] == NEW_ENTRY) {
                    ...
                    $this->syncCashflowEntriesForSale((int) $data['sale_id_num']);

                    $data['barcode'] = $this->barcode_lib->generate_receipt_barcode($data['sale_id']);
                    $this->sale_lib->clear_all();
                    if($this->sale_lib->isCustomerCopy()) {

                        $config['receipt_template'] = 'customer_invoice';
    ...
    private function _reload(array $data = []): ResponseInterface|string    // TODO: Hungarian notation
    {
        ...
        $cash_rounding = $this->sale_lib->reset_cash_rounding();
        $data['print_customer_copy'] = $this->sale_lib->isCustomerCopy();
        ...
        $active_cashflow_accounts = $this->cashflow_account->getActiveOptions();
        $data['has_cashflow_accounts'] = !empty($active_cashflow_accounts);
        $data['cashflow_accounts'] = ['' => lang('Common.none_selected_text')] + $active_cashflow_accounts;
        $data['default_cashflow_account_id'] = $this->getDefaultCashflowAccountId($active_cashflow_accounts);
     ...
     * @noinspection PhpUnused
     */
    public function getReceipt(int $sale_id, bool $customer_copy = false): string
    {
        $data = $this->_load_sale_data($sale_id);
        $this->sale_lib->clear_all();
        if ($customer_copy) {
            $data['is_customer_copy'] = true;
            // Fetch saved cart from database for customer copy
            $saved_cart = $this->sale->get_cart($sale_id);
            // return serialize($saved_cart);
            if ($saved_cart) {
                $data['cart'] = $saved_cart;
            }
        }
        return view('sales/receipt', $data);
    }
     ...
     *
     * @param int $sale_id
     * @param bool $customer_copy
     * @return string
     * @noinspection PhpUnused
     */
    public function getInvoice(int $sale_id, bool $customer_copy = false): string
    {
        $data = $this->_load_sale_data($sale_id);
        $this->sale_lib->clear_all();

        if ($customer_copy) {
            $data['customer_copy'] = true;
            $data['is_customer_copy'] = true;
            // Fetch saved cart from database for customer copy
            $saved_cart = $this->sale->get_cart($sale_id);
            if ($saved_cart) {
    ...
    public function postSave(int $sale_id = NEW_ENTRY): ResponseInterface
    {
        ...
        for ($i = 0; $i < $number_of_payments; ++$i) {
            ...
            $cashflow_account_id = $this->nullableInt($this->request->getPost("cashflow_account_id_$i"));

            $cash_adjustment = $payment_type == lang('Sales.cash_adjustment') ? CASH_ADJUSTMENT_TRUE : CASH_ADJUSTMENT_FALSE;

            if (!$cash_adjustment && $payment_amount != 0) {
                if (empty($cashflow_accounts)) {
                    return $this->response->setJSON(['success' => false, 'message' => lang('Sales.cashflow_no_active_accounts'), 'id' => $sale_id]);
                }

                if ($cashflow_account_id === null || !array_key_exists($cashflow_account_id, $cashflow_accounts)) {
                    return $this->response->setJSON(['success' => false, 'message' => lang('Sales.cashflow_account_required'), 'id' => $sale_id]);
                }
            }
        ...
        $cashflow_account_id_new = $this->nullableInt($this->request->getPost('cashflow_account_id_new'));
        ...
        if ($payment_type != PAYMENT_TYPE_UNASSIGNED && !empty($payment_amount_new)) {
            ...
            if ($payment_type == lang('Sales.cash_adjustment')) {
                ...
                if ($cashflow_account_id_new === null || !array_key_exists($cashflow_account_id_new, $cashflow_accounts)) {
                    return $this->response->setJSON(['success' => false, 'message' => lang('Sales.cashflow_account_required'), 'id' => $sale_id]);
                }
    ...
    private function getDefaultCashflowAccountId(array $cashflow_accounts): ?int
    {
        if (empty($cashflow_accounts)) {
            return null;
        }

        $first_key = array_key_first($cashflow_accounts);
        if ($first_key === null) {
            return null;
        }

        $account_id = (int) $first_key;
        return $account_id > 0 ? $account_id : null;
    }

    private function findPaymentKeyByType(array $payments, string $payment_type): ?string
    {
    ...
    private function syncCashflowEntriesForSale(int $sale_id): void
    {
        $sale_info = $this->sale->get_info($sale_id)->getRowArray();
        ...
        $sale_time = $sale_info['sale_time'] ?? date('Y-m-d H:i:s');
        $sale_type = (int) $this->sale->get_sale_type($sale_id);
        $is_return_sale = $sale_type === SALE_TYPE_RETURN;
        $customer_id = $this->nullableInt($sale_info['customer_id'] ?? null);
        $employee_id = (int) ($sale_info['employee_id'] ?? $this->employee->get_logged_in_employee_info()->person_id);

        $active_payment_ids = [];
        foreach ($this->sale->get_sale_payments($sale_id)->getResultArray() as $payment) {
            $sale_payment_id = (int) ($payment['payment_id'] ?? 0);
            if ($sale_payment_id <= 0) {
                continue;
            }

            $active_payment_ids[] = $sale_payment_id;

            $is_cash_adjustment = (int) ($payment['cash_adjustment'] ?? CASH_ADJUSTMENT_FALSE) === CASH_ADJUSTMENT_TRUE;
            $account_id = $this->nullableInt($payment['cashflow_account_id'] ?? null);
            $payment_net_amount = (float) ($payment['payment_amount'] ?? 0) - (float) ($payment['cash_refund'] ?? 0);

            if ($is_cash_adjustment || $account_id === null || abs($payment_net_amount) < 0.00001) {
                $this->cashflow_entry
            ...
            $entry_type = ($is_return_sale || $payment_net_amount < 0) ? 'outcome' : 'income';
            $entry_data = [
                'entry_date' => $sale_time,
                'entry_type' => $entry_type,
                'amount' => abs($payment_net_amount),
                'description' => 'Sale POS ' . $sale_id . ' - ' . ($payment['payment_type'] ?? ''),
                'status' => 'posted',
            ...
            $existing_entry = $this->cashflow_entry->where('sale_payment_id', $sale_payment_id)->first();
            if ($existing_entry) {
                $this->cashflow_entry->update((int) $existing_entry['entry_id'], $entry_data);
            } else {
                $this->cashflow_entry->insert($entry_data);
            }
        }

        $stale_query = $this->cashflow_entry
            ->where('sale_id', $sale_id)
            ->where('sale_payment_id IS NOT NULL', null, false);

        if (!empty($active_payment_ids)) {
            $stale_query->whereNotIn('sale_payment_id', $active_payment_ids);
        }

        $stale_query->set(['deleted' => 1])->update();
    }
