...
use App\Models\Reports\Money_transactions;
...
class Reports extends Secure_Controller
{
    ...
    {
        $this->clearCache();
        $inputs = $this->buildReportInputs($start_date, $end_date, $sale_type, $location_id);

        $report_data = $this->summary_sales->getData($inputs);
        $summary = $this->summary_sales->getSummaryData($inputs);

        $tabular_data = $this->formatSalesData($report_data);

        return $this->renderTabularReport(
            lang('Reports.sales_summary_report'),
            $start_date,
            $end_date,
            $this->summary_sales->getDataColumns(),
            $tabular_data,
            $summary
        );
    }
    ...
    public function summary_categories(string $start_date, string $end_date, string $sale_type, string $location_id = 'all'): string
    {
        ...
        $tabular_data = $this->formatCategoriesData($report_data);

        return $this->renderTabularReport(
            lang('Reports.categories_summary_report'),
            $start_date,
            $end_date,
            $this->summary_categories->getDataColumns(),
            $tabular_data,
            $summary
        );
    }
    ...
    public function summary_customers(string $start_date, string $end_date, string $sale_type, string $location_id = 'all'): string
    {
        ...
        $tabular_data = $this->formatCustomersData($report_data);

        return $this->renderTabularReport(
            lang('Reports.customers_summary_report'),
            $start_date,
            $end_date,
            $this->summary_customers->getDataColumns(),
            $tabular_data,
            $summary
        );
    }
    ...
    /**
     * Build standard report inputs array.
     */
    private function buildReportInputs(string $start_date, string $end_date, string $sale_type, string $location_id): array
    {
        return [
     ...
     */
    private function renderTabularReport(string $title, string $start_date, string $end_date, array $headers, array $data, array $summary): string
    {
    ...
    /**
     * Format sales summary data rows.
     */
    private function formatSalesData(array $report_data): array
    {
        $tabular_data = [];
    ...
    /**
     * Format categories summary data rows.
     */
    private function formatCategoriesData(array $report_data): array
    {
        $tabular_data = [];
        foreach ($report_data as $row) {
            $tabular_data[] = [
    ...
    /**
     * Format customers summary data rows.
     */
    private function formatCustomersData(array $report_data): array
    {
        $tabular_data = [];
        foreach ($report_data as $row) {
            $tabular_data[] = [
    ...
    /**
     * Gets the items flow detailed report input view.
     *
     * @return string
     * @noinspection PhpUnused
     */
    public function inventory_items_flow_input(): string
    {
        $this->clearCache();
        ...
        return view('reports/inventory_items_flow_input', $data);
    }

    /**
     * Detailed item inventory movements report.
     *
     * @param string $start_date
     * @param string $end_date
     * @param string $location_id
     * @return string
     */
    public function inventory_items_flow(string $start_date, string $end_date, string $location_id = 'all'): string
    {
        $this->clearCache();

        $db = db_connect();
        $builder = $db->table('inventory');
        $builder->select('inventory.trans_date');
        $builder->select('inventory.trans_inventory');
        $builder->select('inventory.trans_comment');
        $builder->select('items.item_number');
        $builder->select('items.name AS item_name');
        $builder->select('stock_locations.location_name');
        $builder->select('people.first_name');
        $builder->select('people.last_name');
        $builder->join('items', 'items.item_id = inventory.trans_items', 'left');
        $builder->join('stock_locations', 'stock_locations.location_id = inventory.trans_location', 'left');
        $builder->join('employees', 'employees.person_id = inventory.trans_user', 'left');
        $builder->join('people', 'people.person_id = employees.person_id', 'left');
        $builder->where('inventory.trans_items IS NOT NULL');
        $builder->where('inventory.trans_items !=', 0);

        // Use parameter binding for date filtering to prevent SQL injection
        if (empty($this->config['date_or_time_format'])) {
        	$builder->where('DATE_FORMAT(inventory.trans_date, "%Y-%m-%d") >=', $start_date);
        	$builder->where('DATE_FORMAT(inventory.trans_date, "%Y-%m-%d") <=', $end_date);
        } else {
        	$builder->where('inventory.trans_date >=', rawurldecode($start_date));
        	$builder->where('inventory.trans_date <=', rawurldecode($end_date));
        }

        if ($location_id !== 'all') {
        	$builder->where('inventory.trans_location', (int) $location_id);
        }

        $builder->orderBy('inventory.trans_date', 'DESC');
        ...
        foreach ($report_data as $row) {
            $quantity = (float) $row['trans_inventory'];
            $net += $quantity;

            if ($quantity >= 0) {
                $total_in += $quantity;
                $movement_type = lang('Reports.movement_in');
            } else {
                $total_out += abs($quantity);
                $movement_type = lang('Reports.movement_out');
            }

            $employee_name = trim(($row['first_name'] ?? '') . ' ' . ($row['last_name'] ?? ''));

            $tabular_data[] = [
                'date'           => to_datetime(strtotime((string) $row['trans_date'])),
                'item_number'    => $row['item_number'] ?? '',
                'item_name'      => $row['item_name'] ?? '',
                'stock_location' => $row['location_name'] ?? '',
                'movement_type'  => $movement_type,
                'quantity'       => to_quantity_decimals($quantity),
                'comments'       => $row['trans_comment'] ?? '',
        ...
        $headers = [
            ...
            ['movement_type'  => lang('Reports.movement_type')],
            ['quantity'       => lang('Reports.quantity')],
            ['comments'       => lang('Reports.comments')],
            ['employee'       => lang('Reports.employee')]
        ];

        $data = [
            'title'        => lang('Reports.inventory_items_flow_report'),
            'subtitle'     => $this->_get_subtitle_report(['start_date' => $start_date, 'end_date' => $end_date]),
            'headers'      => $headers,
            'data'         => $tabular_data,
            'summary_data' => [
                'total_quantity' => to_quantity_decimals($net)
            ]
        ];
     ...
     */
    public function moneytransactions_input(): string
    {
        if (!$this->employee->has_grant('reports_money_transactions', $this->employee->get_logged_in_employee_info()->person_id)) {
            return view('no_access', ['module_name' => lang('Module.reports'), 'permission_id' => 'reports_money_transactions']);
        }
        ...
        return view('reports/money_transactions_input', [
            'customers' => $customers,
            'suppliers' => $suppliers
        ]);
    }

    public function moneytransactions(string $start_date, string $end_date, string $entity_type, string $entity_id): string
    {
        if (!$this->employee->has_grant('reports_money_transactions', $this->employee->get_logged_in_employee_info()->person_id)) {
            return view('no_access', ['module_name' => lang('Module.reports'), 'permission_id' => 'reports_money_transactions']);
        }
        ...
        $entityType = $entity_type === 'supplier' ? 'supplier' : 'customer';
        $entityId = (int) $entity_id;
        $entityName = '';

        if ($entityType === 'supplier') {
            $supplier = $this->supplier->get_info($entityId);
            $entityName = $supplier->company_name ?? '';
        } else {
            $customer = $this->customer->get_info($entityId);
            $entityName = trim(($customer->first_name ?? '') . ' ' . ($customer->last_name ?? ''));
        }

        $report = model(Money_transactions::class);
        $inputs = [
            'start_date' => $start_date,
            'end_date' => $end_date,
        ...
        $reportData = $report->getData($inputs);
        $summary = $report->getSummaryData($inputs);
        $balanceLabel = ($summary['net_balance'] ?? 0) >= 0
            ? lang('Reports.party_owes_store')
            : lang('Reports.store_owes_party');

        $summaryLabel = $entityType === 'supplier'
            ? lang('Reports.total_purchases')
            : lang('Reports.total_sales');
        $listLabel = $entityType === 'supplier'
            ? lang('Reports.receivings')
            : lang('Reports.trans_sales');
        $detailsHeaders = [
            ['name' => lang('Reports.item_name')],
            ['quantity' => lang('Reports.quantity')],
            ['total' => lang('Reports.total'), 'sorter' => 'number_sorter']
        ];

        return view('reports/money_transactions', [
            'title' => lang('Reports.money_transactions'),
            'subtitle' => $this->_get_subtitle_report(['start_date' => $start_date, 'end_date' => $end_date]),
            ...
            'payments_data' => $reportData['payments'] ?? [],
            'details_headers' => $detailsHeaders,
            'details_data' => $reportData['details'] ?? [],
            'config' => $this->config
        ]);
    }
