...
class Config extends Secure_Controller
{
    ...
    {
        $license = [];
        $license[] = $this->getOsposLicense();

        $flags = $this->scanLicenseDirectory($license);

        if ($flags['composer']) {
            $license[] = $this->getComposerLicenses();
        }

        if ($flags['npmProd']) {
            $license[] = $this->getNpmLicenses('npm-prod.LICENSES', 'NPM Production Libraries');
        }

        if ($flags['npmDev']) {
            $license[] = $this->getNpmLicenses('npm-dev.LICENSES', 'NPM Development Libraries');
        }
    ...
    /**
     * Get the main OSPOS license.
     */
    private function getOsposLicense(): array
    {
        $title = 'Open Source Point Of Sale ' . config('App')->application_version;

        if (file_exists('license/LICENSE')) {
            $text = file_get_contents('license/LICENSE', false, null, 0, 3000);
        } else {
            $text = 'LICENSE file must be in OSPOS license directory. You are not allowed to use OSPOS application until the distribution copy of LICENSE file is present.';
        }
    ...
    /**
     * Scan the license directory and collect version-based licenses.
     * @return array Flags indicating which special license files were found.
     */
    private function scanLicenseDirectory(array &$license): array
    {
        $flags = ['composer' => false, 'npmProd' => false, 'npmDev' => false];
        ...
        foreach ($dir as $fileinfo) {
            ...
            if ($fileinfo->getExtension() == 'version') {
                $license[] = $this->parseVersionLicense($fileinfo);
    ...
    /**
     * Parse a version-based license file.
     */
    private function parseVersionLicense(DirectoryIterator $fileinfo): array
    {
        $basename = 'license/' . $fileinfo->getBasename('.version');
        $title = file_get_contents($basename . '.version', false, null, 0, 100);
        $license_text_file = $basename . '.license';

        if (file_exists($license_text_file)) {
            $text = file_get_contents($license_text_file, false, null, 0, 2000);
        } else {
            $text = $license_text_file . ' file is missing';
        }
    ...
    /**
     * Get Composer licenses from the LICENSES file.
     */
    private function getComposerLicenses(): array
    {
        $title = 'Composer Libraries';
        $text = '';
    ...
    /**
     * Get NPM licenses from a LICENSES file.
     */
    private function getNpmLicenses(string $filename, string $title): array
    {
        $text = '';
        $file = file_get_contents('license/' . $filename);
        $array = json_decode($file, true);

        foreach ($array as $dependency) {
            $text .= "library: {$dependency['name']}\n";
            $text .= "authors: {$dependency['author']}\n";
            $text .= "website: {$dependency['homepage']}\n";
            $text .= "version: {$dependency['installedVersion']}\n";
            $text .= "license: {$dependency['licenseType']}\n";
            $text .= "\n";
        }

        return ['title' => $title, 'text' => rtrim($text, "\n")];
    }
    ...
    public function getIndex(): string
    {
        ...
        $data['licenses'] = $this->getLicenses();
