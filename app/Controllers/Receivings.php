...
class Receivings extends Secure_Controller
{
    ...
    public function postAdd(): string
    {
        ...
        return $this->reloadReceiving($data);
    }
     ...
     * @return string
     */
    private function reloadReceiving(array $data = []): string
    {
        $data = $this->buildReceivingBaseData($data);
        $data = $this->buildReceivingStockLocationData($data);
        $data = $this->buildReceivingSupplierData($data);
    ...
    /**
     * Build base data for receiving view.
     */
    private function buildReceivingBaseData(array $data): array
    {
        $data['cart'] = $this->receiving_lib->get_cart();
    ...
    /**
     * Build stock location data for receiving view.
     */
    private function buildReceivingStockLocationData(array $data): array
    {
    ...
    /**
     * Build supplier data for receiving view.
     */
    private function buildReceivingSupplierData(array $data): array
    {
        $supplier_id = $this->receiving_lib->get_supplier();
        ...
        if ($supplier_id != -1) {
            ...
            $data['supplier_email'] = $supplier_info->email;
            $data['supplier_address'] = $supplier_info->address_1;
            $data['supplier_location'] = $this->formatSupplierLocation($supplier_info);
        }
    ...
    /**
     * Format supplier location string.
     */
    private function formatSupplierLocation(object $supplier_info): string
    {
        if (!empty($supplier_info->zip) || !empty($supplier_info->city)) {
            return $supplier_info->zip . ' ' . $supplier_info->city;
        }
        return '';
    }
