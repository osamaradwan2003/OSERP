...
class Items extends Secure_Controller
{
    ...
    public function getIndex(): string
    {
        ...
        $show_all_stores_columns = ((int) $data['stock_location'] === -1);
        $data['show_all_stores_columns'] = $show_all_stores_columns;
        $data['table_headers'] = get_items_manage_table_headers($show_all_stores_columns);
        $data['table_headers_all'] = get_items_manage_table_headers(true);
        $data['table_headers_single'] = get_items_manage_table_headers(false);
        $data['stock_locations'] = ['-1' => lang('Reports.all')] + $this->stock_location->get_allowed_locations();
    ...
    public function getSearch(): ResponseInterface
    {
        ...
        $stock_location = (int) ($this->request->getGet('stock_location', FILTER_SANITIZE_NUMBER_INT) ?? -1);
        $show_all_stores_columns = ($stock_location === -1);
        $sort = $this->sanitizeSortColumn(item_headers($show_all_stores_columns), $params['sort'], 'item_id');
        ...
        $request_filters_input = $this->request->getGet('filters', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        if (!is_array($request_filters_input)) {
            $request_filters_input = empty($request_filters_input) ? [] : [$request_filters_input];
        }
        $request_filters = array_fill_keys($request_filters_input, true);
        $filters = array_merge($filters, $request_filters);
        if ($show_deleted) {
            $filters['is_deleted'] = true;
        }

        $items = $this->item->search($params['search'], $filters, $params['limit'], $params['offset'], $sort, $order);
        $total_rows = $this->item->get_found_rows($params['search'], $filters);
        $data_rows = [];

        foreach ($items->getResult() as $item) {
            $data_rows[] = get_item_data_row($item, $show_all_stores_columns);
    ...
    {
        $item_location = (int) $this->item_lib->get_item_location();
        $show_all_stores_columns = ($item_location === -1);
        $item_ids_array = explode(':', $item_ids);
        $fallback_location = $item_location;
        if ($show_all_stores_columns) {
            $allowed_locations = $this->stock_location->get_allowed_locations();
            $fallback_location = !empty($allowed_locations) ? (int) array_key_first($allowed_locations) : 0;
        }
        $item_infos = $show_all_stores_columns
            ? $this->item->get_multiple_info($item_ids_array, $fallback_location)
            : $this->item->get_multiple_info($item_ids_array, $item_location);

        $result = [];

        foreach ($item_infos->getResult() as $item_info) {
            $result[$item_info->item_id] = get_item_data_row($item_info, $show_all_stores_columns);
        }
     ...
     * @return string
     */
    public function getView(int $item_id = NEW_ENTRY): string
    {
        $item_id ??= NEW_ENTRY;
        $data = $item_id === NEW_ENTRY ? [] : [];

        $data = $this->buildItemViewBaseData($item_id, $data);

        $item_info = $this->item->get_info($item_id);
        $use_destination_based_tax = (bool)$this->config['use_destination_based_tax'];

        $data = $this->buildItemViewConfigData($data);
        $data = $this->buildCategoryDropdownData($item_info, $data);

        if ($item_id === NEW_ENTRY) {
            $item_info = $this->initializeNewItemDefaults($item_info, $use_destination_based_tax);
            $data['default_tax_1_rate'] = $this->config['default_tax_1_rate'];
            $data['default_tax_2_rate'] = $this->config['default_tax_2_rate'];
        }

        $data['standard_item_locked'] = $this->isStandardItemLocked($data, $item_info);
        $data['item_info'] = $item_info;

        $data = $this->buildSupplierData($item_info, $data);
        $data['hsn_code'] = $data['include_hsn'] ? $item_info->hsn_code : '';

        $data = $this->buildTaxCategoryData($item_info, $use_destination_based_tax, $data);
        $data = $this->buildImageData($item_info, $data);
        $data = $this->buildStockLocationData($item_id, $data);
        $data = $this->buildLowSellItemData($item_id, $item_info, $data);

        return view('items/form', $data);
    }

    /**
     * Build base data for item view.
     */
    private function buildItemViewBaseData(int $item_id, array $data): array
    {
    ...
    /**
     * Build configuration-based data for item view.
     */
    private function buildItemViewConfigData(array $data): array
    {
        $item_info = $this->item->get_info(NEW_ENTRY);
        $data['allow_temp_item'] = ($data['allow_temp_item'] === 1 && $item_info->item_type != ITEM_TEMP) ? 0 : 1;
    ...
    /**
     * Build category dropdown data if enabled.
     */
    private function buildCategoryDropdownData(object $item_info, array $data): array
    {
        if ($data['category_dropdown'] === '1') {
            ...
            $category_options = array_combine($category_options, $category_options);
     ...
     */
    private function initializeNewItemDefaults(object $item_info, bool $use_destination_based_tax): object
    {
        $item_info->receiving_quantity = 1;
        $item_info->reorder_level = 1;
        $item_info->item_type = ITEM;
    ...
    /**
     * Check if standard item is locked.
     */
    private function isStandardItemLocked(array $data, object $item_info): bool
    {
        return (
    ...
    /**
     * Build supplier dropdown data.
     */
    private function buildSupplierData(object $item_info, array $data): array
    {
    ...
    /**
     * Build tax category data for destination-based tax.
     */
    private function buildTaxCategoryData(object $item_info, bool $use_destination_based_tax, array $data): array
    {
        if ($use_destination_based_tax) {
    ...
    /**
     * Build image data for item view.
     */
    private function buildImageData(object $item_info, array $data): array
    {
        ...
        if ($item_info->pic_filename != null) {
            ...
            $data['image_path'] = sizeof($images) > 0
                ? base_url(implode('/', array_map('rawurlencode', explode('/', ltrim($images[0], './')))))
                : '';
        } else {
            $data['image_path'] = '';
        }
    ...
    /**
     * Build stock location data with quantities.
     */
    private function buildStockLocationData(int $item_id, array $data): array
    {
    ...
    /**
     * Build low sell item data.
     */
    private function buildLowSellItemData(int $item_id, object $item_info, array $data): array
    {
    ...
    {
        $items_to_restore = $this->request->getPost('ids');

        if (!is_array($items_to_restore) || empty($items_to_restore)) {
            return $this->response->setJSON(['success' => false, 'message' => lang('Cashflow.nothing_selected')]);
        }

        $restored_count = 0;
        foreach ($items_to_restore as $item_id) {
            if ($this->item->undelete((int) $item_id)) {
                ++$restored_count;
            }
        }

        if ($restored_count === count($items_to_restore)) {
