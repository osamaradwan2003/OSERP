 ...
 * @var array $cashflow_accounts
 * @var int|null $default_cashflow_account_id
 * @var bool $has_cashflow_accounts
 * @var bool $pos_mode
 * @var array $payments
    ...
    <div class="panel-body">
        ...
        <?php if (count($cart) > 0) { // Only show this part if there are Items already in the register ?>
            ...
            <div id="payment_details">
                <?php if (!$has_cashflow_accounts) { ?>
                    <div class="alert alert-dismissible alert-danger"><?= lang('Sales.cashflow_no_active_accounts') ?></div>
                <?php } ?>
                <?php if ($payments_cover_total) { // Show Complete sale button instead of Add Payment if there is no amount due left ?>
                    <?= form_open("$controller_name/addPayment", ['id' => 'add_payment_form', 'class' => 'form-horizontal']) ?>
                        <table class="sales_table_100">
                            ...
                            <tr>
                                <td><?= lang('Cashflow.account') ?></td>
                                <td>
                                    <?= form_dropdown('cashflow_account_id', $cashflow_accounts, $default_cashflow_account_id, ['id' => 'cashflow_account_id', 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-default btn-sm', 'data-width' => 'fit', 'disabled' => 'disabled']) ?>
                                </td>
                            </tr>
                        </table>
                    <?= form_close() ?>
                    ...
                    <?= form_open("$controller_name/addPayment", ['id' => 'add_payment_form', 'class' => 'form-horizontal']) ?>
                        <table class="sales_table_100">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $cashflow_account_attributes = ['id' => 'cashflow_account_id', 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-default btn-sm', 'data-width' => 'fit'];
                                    if (!$has_cashflow_accounts) {
                                        $cashflow_account_attributes['disabled'] = 'disabled';
                                    }
                                    ?>
                                    <?= form_dropdown('cashflow_account_id', $cashflow_accounts, $default_cashflow_account_id, $cashflow_account_attributes) ?>
                                </td>
                            </tr>
                        </table>
                    <?= form_close() ?>

                    <div class="btn btn-sm btn-success pull-right <?= !$has_cashflow_accounts ? 'disabled' : '' ?>" id="add_payment_button" tabindex="<?= ++$tabindex ?>">
                    ...
                    <table class="sales_table_100" id="register">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 45%;"><?= lang(ucfirst($controller_name) . '.payment_type') ?></th>
                                <th style="width: 25%;"><?= lang('Cashflow.account') ?></th>
                        ...
                        <tbody id="payment_contents">
                            <?php foreach ($payments as $payment_id => $payment) { ?>
                                <?php
                                $payment_account_id = (int) ($payment['cashflow_account_id'] ?? 0);
                                $payment_account_name = $payment_account_id > 0 && isset($cashflow_accounts[$payment_account_id]) ? $cashflow_accounts[$payment_account_id] : lang('Common.none_selected_text');
                                ?>
                                <tr>
                                    ...
                                    <td><?= esc($payment_account_name) ?></td>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-xs-6">
                                <label for="print_customer_copy" class="control-label checkbox">
                                    <?= form_checkbox(['name' => 'print_customer_copy', 'id' => 'print_customer_copy', 'value' => 1, 'checked' => $print_customer_copy]) ?>
                                    <?= lang(ucfirst($controller_name) . '.print_customer_copy') ?>
                                </label>
                            </div>
                            <?php if (!empty($customer_email)) { ?>
                                <div class="col-xs-6">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#print_customer_copy').change(function() {
            $.post("<?= esc(site_url("$controller_name/setPrintCustomerCopy")) ?>", {
                print_customer_copy: $(this).is(':checked')
            });
        });
