...
$employee_model = model(\App\Models\Employee::class);
$logged_person_id = (int) ($user_info->person_id ?? 0);
$has_accountant_grant = $logged_person_id > 0 && $employee_model->has_grant('accountant', $logged_person_id);
$has_cashflow_grant = $logged_person_id > 0 && $employee_model->has_grant('cashflow', $logged_person_id);
$has_cashflow_categories_grant = $logged_person_id > 0 && (
    $employee_model->has_grant('cashflow_categories', $logged_person_id) ||
    $employee_model->has_grant('cashflow_manage_categories', $logged_person_id)
);
$accountant_in_allowed_modules = false;
$cashflow_in_allowed_modules = false;
$cashflow_categories_in_allowed_modules = false;
...
if (!empty($allowed_modules)) {
    foreach ($allowed_modules as $allowed_module) {
        if (($allowed_module->module_id ?? '') === 'accountant') {
            $accountant_in_allowed_modules = true;
        }
        if (($allowed_module->module_id ?? '') === 'cashflow') {
            $cashflow_in_allowed_modules = true;
        }
        if (($allowed_module->module_id ?? '') === 'cashflow_categories') {
            $cashflow_categories_in_allowed_modules = true;
        }
    }
}

if ($has_accountant_grant && !$accountant_in_allowed_modules) {
    $accountant_module = new \stdClass();
    $accountant_module->module_id = 'accountant';
    $allowed_modules[] = $accountant_module;
}

if ($has_cashflow_grant && !$cashflow_in_allowed_modules) {
    $cashflow_module = new \stdClass();
    $cashflow_module->module_id = 'cashflow';
    $allowed_modules[] = $cashflow_module;
}

if ($has_cashflow_categories_grant && !$cashflow_categories_in_allowed_modules) {
    $cashflow_categories_module = new \stdClass();
    $cashflow_categories_module->module_id = 'cashflow_categories';
    $allowed_modules[] = $cashflow_categories_module;
}
?>
...
<head>
    ...
    <?php if (ENVIRONMENT == 'development' || get_cookie('debug') == 'true' || $request->getGet('debug') == 'true') : ?>
        ...
        <script src="resources/js/jspdf-41ded5e6e0.umd.js"></script>
    ...
    <?php else : ?>
        ...
        <script src="resources/opensourcepos-4410e22a32.min.js"></script>
        <!-- endinject -->
    <?php endif; ?>
        ...
        <div class="navbar navbar-default" role="navigation">
            <div class="container">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand hidden-sm" href="<?= site_url() ?>">ERP</a>
                </div>
